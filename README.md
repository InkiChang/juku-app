...
[![Android APK](https://github.com/InkiChang/juku-app/actions/workflows/android-apk.yml/badge.svg)](https://github.com/InkiChang/juku-app/actions/workflows/android-apk.yml)
...
以下为 Android 开发测试阶段的界面截图，界面和播放能力以实际设备和后端返回内容为准。

| 客户端 | 播放页面 |
| --- | --- |
| ![模拟器界面](docs/screenshots/emulator-current.png) | ![播放页面](docs/screenshots/final-player.png) |
| ![全屏播放前](docs/screenshots/fs-before.png) | ![全屏播放调整后](docs/screenshots/fs-after-beforefix.png) |
...
6. 首发版本为 `0.0.1`，当前版本为 `0.0.2`。版本信息维护在 `package.json` 与 `version.json`；版本 tag 使用 `v` 前缀。
...
- 推送 `v*` tag：工作流校验 tag 与 `version.json`/`package.json` 一致，构建可安装的 Debug APK 并创建 GitHub Release 附件。
...
本次 `v0.0.2` 是使用 CI 临时调试密钥签名的测试包，不是 Play Store 正式发布包。不同环境构建的 Debug APK 签名可能不同；升级安装失败时，请先备份本地数据，再卸载旧包。正式发布需维护者配置固定发布签名和 CI 签名流程，不要将 keystore 或密码提交到仓库。

## 版本

当前版本：`0.0.2`。构建产物使用版本号命名，Android `versionCode` 随版本递增。本仓库目前未声明开源许可证，公开可读不代表授予复制、修改或分发许可；许可由维护者另行确定。
